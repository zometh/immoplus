...
<jsp:include page="../header.jsp">
    <jsp:param name="title" value="Edit d'un immeuble"/>
</jsp:include>
...
<div class="flex-1">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-lg shadow-md">
            <div class="p-6">
                <h2 class="text-xl font-semibold mb-4">Edit un immeuble</h2>
                <form method="post" action="${pageContext.request.contextPath}/immeubles?action=update&&id=<%=immeuble.getId()%>" class="space-y-6" enctype="multipart/form-data">
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-gray-700">Nom de l'immeuble</label>
                        <input value="<%=immeuble.getNom()%>" name="nom" type="text" placeholder="Entrez le nom de l'immeuble"
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                               required/>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-gray-700">Adresse</label>
                        <input value="<%=immeuble.getAdresse()%>" name="adresse" type="text" placeholder="Entrez l'adresse"
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                               required/>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-gray-700">Description</label>
                        <input value="<%=immeuble.getDescription()%>" name="description" type="text" placeholder="Entrez la description"
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                               required/>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-gray-700">Equipements</label>
                        <input value="<%=immeuble.getEquipements()%>" name="equipements" type="text" placeholder="Saisissez les équipements disponibles"
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                               required/>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-gray-700">Nombre de pièces</label>
                        <input min="<%=immeuble.getNbPieces()%>" value="<%=immeuble.getNbPieces()%>" name="nbPieces" type="number" placeholder="Saisissez le nombre de pièces"
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                               required/>
                    </div>
